...
SUPPORT_KEYWORDS = {
    'access': ['login', 'access', 'sign in', 'sign-in', 'password', 'account', 'credentials', 'otp', 'code', 'pin', 'verification', 'verify'],
    'technical': ['error', 'bug', 'issue', 'crash', 'problem', 'not working', 'broken', 'failed', 'failure', 'malfunction', 'glitch'],
    'feature': ['feature', 'request', 'function', 'functionality', 'capability', 'option', 'button', 'add'],
    'payment': ['payment', 'billing', 'charge', 'invoice', 'refund', 'subscription', 'credit card', 'transaction', 'cost', 'price'],
    'account': ['account', 'profile', 'settings', 'password', 'email', 'username', 'user', 'registration'],
    'data': ['data', 'information', 'file', 'upload', 'download', 'export', 'import', 'sync', 'receive', 'receiving', 'sent', 'sending'],
    'support': ['help', 'support', 'assist', 'question', 'assistance', 'urgent', 'critical', 'contact'],
    'performance': ['slow', 'lag', 'delay', 'freeze', 'hang', 'performance', 'speed', 'loading'],
    'integration': ['integration', 'connect', 'api', 'sync', 'plugin', 'extension', 'module'],
    'notification': ['notification', 'alert', 'message', 'email', 'reminder', 'sms', 'text']
}
...
def validate_ticket_id(ticket_id):
    ...
    if len(ticket_id) > 50:
        return {
            'is_valid': False,
            'error_message': "❌ Ticket ID cannot exceed 50 characters.",
            'error_type': 'too_long'
        }
    ...
    if not re.match(r'^[a-zA-Z0-9\-_]+$', ticket_id):
        return {
            'is_valid': False,
            'error_message': "❌ Ticket ID can only contain letters, numbers, hyphens, and underscores (e.g., TKT-001).",
    ...
    if re.match(r'^\d+$', ticket_id):
        return {
            'is_valid': False,
            'error_message': "❌ Ticket ID should have a prefix (e.g., TKT-001 instead of just 001).",
            'error_type': 'invalid_format'
        }
...
def validate_subject(subject):
    ...
    if is_text_too_long(subject, max_length=150):
        return {
            'is_valid': False,
            'error_message': "❌ Subject cannot exceed 150 characters.",
            'error_type': 'too_long'
        }
    ...
    if has_special:
        ...
        return {
            'is_valid': False,
            'error_message': f"❌ Invalid characters not allowed: {invalid_chars_str}. Please use alphanumeric, spaces, and common punctuation only.",
    ...
    if is_gibberish:
        return {
            'is_valid': False,
            'error_message': f"❌ {reason}. Please provide a meaningful subject.",
    ...
    if has_inappropriate:
        return {
            'is_valid': False,
            'error_message': f"❌ Please avoid using offensive language in your subject.",
    ...
    if has_irrelevant:
        return {
            'is_valid': False,
            'error_message': f"❌ Subject should be about a customer support issue, not about {irrelevant_words[0]}. Please describe your support request.",
    ...
    if not has_keywords:
        return {
            'is_valid': False,
            'error_message': "❌ Subject must contain keywords related to customer support (e.g., 'login issue', 'payment problem', 'feature request', 'error', 'bug', etc.).",
...
def validate_description(description):
    ...
    if is_text_too_long(description, max_length=2000):
        return {
            'is_valid': False,
            'error_message': "❌ Description cannot exceed 2000 characters.",
            'error_type': 'too_long'
        }
    ...
    if is_gibberish:
        return {
            'is_valid': False,
            'error_message': f"❌ {reason}. Please provide a meaningful description.",
            'error_type': 'gibberish'
        }
    ...
    if has_inappropriate:
        return {
            'is_valid': False,
            'error_message': f"❌ Please avoid using offensive language in your description.",
            'error_type': 'inappropriate_language'
        }
    ...
    if has_irrelevant:
        return {
            'is_valid': False,
            'error_message': f"❌ Description should be about a customer support issue, not about {irrelevant_words[0]}. Please describe your support request.",
            'error_type': 'irrelevant_content'
        }
    ...
    if not has_keywords:
        return {
            'is_valid': False,
            'error_message': "❌ Description must contain keywords related to customer support (e.g., 'issue', 'error', 'problem', 'not working', 'help needed', 'account', 'payment', etc.).",
            'error_type': 'missing_keywords'
        }
